...
def main():
    ...
    _add_engine_args(
        check_build_parser, choices=['libfuzzer', 'afl', 'dataflow','macke'])
    _add_sanitizer_args(
        check_build_parser,
...
def _add_engine_args(parser,choices=('libfuzzer', 'afl', 'honggfuzz', 'dataflow','macke' ,'none')):
...
def _build_image(image_name,no_cache=False,pull=False):
    ...
    build_args+=['-it','%s/%s'%(image_project,image_name),dockerfile_dir]
